...
public class MemberRepository {
    ...
    public void initializeData() {
        Member member1 = Member.createAdmin("Peter", "Pan", "peter_pan@hotmail.com","peter1","passwordpeter");
        Member member2 = Member.createLibrarian("Gandalf","leblanc","gadalf_leblan@hotmail.com","gadalf1","passwordgadalf");
        Member member3 = new Member("951014-523-12","Lisa","Simpson","lisa_simpson@hotmail.com","Evergreen Terrace","742","Springfield", Role.MEMBER,"lisa1","passwordlisa");
    ...
    public List<String> getAllUsernames() {
        return members.values().stream().map(Member::getUsername).collect(Collectors.toList());
    }
    ...
    public Member getMemberByUsername(String username) {
        return members.values().stream().filter(member -> member.getUsername().equals(username)).findFirst().orElse(null);
    }

    public MemberDto createAdmin(MemberDto memberDto) {
        Member admin = Member.createAdmin(memberDto.getLastName(), memberDto.getFirstName(), memberDto.getEmail(), memberDto.getUsername(), memberDto.getPassword());
        members.put(admin.getId(), admin);
        return memberMapper.toDto(admin);
    }

    public MemberDto createLibrarian(MemberDto memberDto) {
        Member librarian = Member.createLibrarian(memberDto.getLastName(), memberDto.getFirstName(), memberDto.getEmail(), memberDto.getUsername(), memberDto.getPassword());
